...
class _CompleteScreenState extends State<CompleteScreen> {
  ...
  Widget build(BuildContext context) {
    final completedTasks = context.select((TodoProvider value) => value);
    return Scaffold(
      backgroundColor: const Color.fromRGBO(24, 24, 24, 1),
      floatingActionButton: Stack(
        children: [
          if (completedTasks.tasks.isNotEmpty)
      ...
      body: Selector<TodoProvider, List>(
        ...
        builder: (context, task, child) => ListView.builder(
          shrinkWrap: true,
          itemCount: completedTasks.completedTask.length,
          itemBuilder: (BuildContext context, int index) {
            return Padding(
              padding: const EdgeInsets.only(left: 16, right: 16, top: 16),
              child: completedTasks.completedTask[index].isDone
                  ? ListTile(
